...
- scanning local Claude Code, Codex, Cursor, Copilot CLI, Copilot for VS Code, Gemini CLI, Antigravity, Amp, Kimi Code, Qwen Code, Droid, OpenCode, Pi, Trae, Kiro (IDE + Kiro-Go / kiro.rs), and Grok Build usage
...
| Kiro | IDE transcripts under `~/.kiro/sessions/` estimate tokens from `contextUsage% ×` the session model's official window (1M / 200k / …) and assistant character counts. Official credit lines have no model or API tokens, so they are not converted at the $0.04 overage rate. Kiro-Go `request_logs.json` and kiro.rs `usage_log.YYYY-MM-DD.jsonl` are ingested as `channel=api` with upstream token counters; cost uses the same model's public API list price. Default dirs include `~/.kiro-go/data` and `~/.kiro.rs`. Extra roots: `scanner.kiroProxyDataDirs`, `KIRO_GO_DATA_DIR`, `KIRO_RS_DATA_DIR`. |
| Grok Build | `$GROK_HOME/sessions/` (default `~/.grok/sessions/`) `updates.jsonl` turn usage, including cached/reasoning splits and `grok-4.6-build`; cost uses xAI list prices. |
